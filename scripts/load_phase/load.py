...
def load(customer, vendor, employee, insurance):

    engine = create_engine(
        f"mysql+pymysql://{db_user}:{db_password}@{db_host}:3306/comfirmed_cleaned_db"
    )
    type(engine)
    with engine.connect() as conn:
        customer.to_sql(
            "customers_confirmed", con=conn, if_exists="replace", index=False
        )
        vendor.to_sql("vendors_confirmed", con=conn, if_exists="replace", index=False)
        employee.to_sql(
            "employees_confirmed", con=conn, if_exists="replace", index=False
        )
        insurance.to_sql(
            "insurance_confirmed", con=conn, if_exists="replace", index=False
        )
